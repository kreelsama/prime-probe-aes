...
This attack uses `rdpmc` instruction to read performance counters inside a cpu so `echo 2 | sudo tee /sys/devices/cpu/rdpmc` to enable execution of `rdpmc` instruction in user space.Then go to `pmc_events` and execute `setup.sh` with root privilege to setup L1d miss connters in cpu.
...
One may also need to alter the CPU numbers in `common.h` which `CPU0` and `CPU1` is the number of CPU the attacker and victim runs on respectively. `CPU0` and `CPU1` must be two defferent logical cores running on a same physical core which be determined by `cat /proc/cpuinfo`.
